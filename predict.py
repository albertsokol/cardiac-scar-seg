...
class PneumothoraxPredictor:
    """
    Predictor class that takes in a chest _img-ray image. Classifies as pneumothorax (pt) or non-pneumothorax. If pt
    predicted, carries out segmentation prediction on the _img-ray for pt localization.

    ...
    Methods
    -------
    predict(fname=None):
        carry out classification on the chest _img-ray and perform segmentation if pt is predicted. Note the optional
    ...
    def __load_dicom(self, fname):
        # Loads a dicom file by filename and updates the _img rgb and grayscale predictor attributes
        if fname is None:
    ...
    def predict(self, fname=None):
        """
        Carry out classification on the chest _img-ray and perform segmentation if pt is predicted. Calls plot method,
        which shows:
        > cxr with radiologist classification, + segmentation map if ground truth was a pt
        > cxr with model classification, + segmentation map if predicted a pt, + confidence in prediction
...
if __name__ == '__main__':
    ...
    pp = PneumothoraxPredictor(fpath='/home/y4tsu/Desktop/data/siim/dicom_train/',
                               csv_path='/home/y4tsu/Desktop/data/siim/train-rle.csv',
                               classifier_path='/home/y4tsu/Desktop/models/Dense169_finetuned_pt_classifier_slow',
                               seg_path='/home/y4tsu/Desktop/models/l3_seg_model_unet_no_fix_slow_lr',
